...
const Invoices = () => {
    // Data from store
    const { invoices: data, deleteInvoice } = useAppStore()
    ...
    const columns = [
        ...
        columnHelper.accessor('status', {
            ...
            cell: (info) => {
                ...
                if (status === 'Unpaid') badgeClass = 'badge-warning'
        ...
        columnHelper.display({
            ...
            cell: (info) => (
                <div className="flex items-center space-x-2">
                    <Link to={`/invoices/${info.row.original.id}`} className="p-1 text-gray-500 hover:text-primary-600 transition-colors" title="View">
                        <FiEye size={18} />
                    </Link>
                    <button className="p-1 text-gray-500 hover:text-primary-600 transition-colors" title="Edit">
                        <FiEdit2 size={18} />
                    </button>
                    <button className="p-1 text-gray-500 hover:text-gray-900 transition-colors" title="PDF">
                        <FiDownload size={18} />
                    </button>
                    <button
                        onClick={() => {
                            if (window.confirm('Are you sure you want to delete this invoice?')) {
                                deleteInvoice(info.row.original.id)
                            }
                        }}
