...
export const useAppStore = create(
    persist(
        (set) => ({
            ...
            addEstimate: (item) => set((state) => {
                const newItem = { ...item, id: `EST-${Date.now()}` };
                syncToSheets('estimates', newItem);
                return { estimates: [...state.estimates, newItem] };
            }),
            updateEstimate: (id, data) => set((state) => ({
                estimates: state.estimates.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addProject: (item) => set((state) => {
                const newItem = { ...item, id: state.projects.length + 1 };
                syncToSheets('projects', newItem);
                return { projects: [...state.projects, newItem] };
            }),
            updateProject: (id, data) => set((state) => ({
                projects: state.projects.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addTask: (item) => set((state) => {
                const newItem = { ...item, id: state.tasks.length + 1 };
                syncToSheets('tasks', newItem);
                return { tasks: [...state.tasks, newItem] };
            }),
            updateTask: (id, data) => set((state) => ({
                tasks: state.tasks.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addLead: (item) => set((state) => {
                const newItem = { ...item, id: state.leads.length + 1 };
                syncToSheets('leads', newItem);
                return { leads: [...state.leads, newItem] };
            }),
            updateLead: (id, data) => set((state) => ({
                leads: state.leads.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addProposal: (item) => set((state) => {
                const newItem = { ...item, id: `PRO-${Date.now()}` };
                syncToSheets('proposals', newItem);
                return { proposals: [...state.proposals, newItem] };
            }),
            updateProposal: (id, data) => set((state) => ({
                proposals: state.proposals.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addContract: (item) => set((state) => {
                const newItem = { ...item, id: `CON-${Date.now()}` };
                syncToSheets('contracts', newItem);
                return { contracts: [...state.contracts, newItem] };
            }),
            updateContract: (id, data) => set((state) => ({
                contracts: state.contracts.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addExpense: (item) => set((state) => {
                const newItem = { ...item, id: state.expenses.length + 1 };
                syncToSheets('expenses', newItem);
                return { expenses: [...state.expenses, newItem] };
            }),
            updateExpense: (id, data) => set((state) => ({
                expenses: state.expenses.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
    ...
    deletePayment: (id) => set((state) => ({ payments: state.payments.filter((i) => i.id !== id) })),
            ...
            addTicket: (item) => set((state) => {
                const newItem = { ...item, id: state.tickets.length + 1 };
                syncToSheets('tickets', newItem);
                return { tickets: [...state.tickets, newItem] };
            }),
            updateTicket: (id, data) => set((state) => ({
                tickets: state.tickets.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            deleteTicket: (id) => set((state) => ({ tickets: state.tickets.filter((i) => i.id !== id) })),

            // --- REPORTS ---
            addReport: (item) => set((state) => {
                const newItem = { ...item, id: state.reports.length + 1 };
                syncToSheets('reports', newItem);
                return { reports: [...state.reports, newItem] };
            }),
            ...
            addResource: (item) => set((state) => {
                const newItem = { ...item, id: state.resources.length + 1 };
                syncToSheets('resources', newItem);
                return { resources: [...state.resources, newItem] };
            }),
            updateResource: (id, data) => set((state) => ({
                resources: state.resources.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addSupplier: (item) => set((state) => {
                const newItem = { ...item, id: state.suppliers.length + 1 };
                syncToSheets('suppliers', newItem);
                return { suppliers: [...state.suppliers, newItem] };
            }),
            updateSupplier: (id, data) => set((state) => ({
                suppliers: state.suppliers.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addSubcontractor: (item) => set((state) => {
                const newItem = { ...item, id: state.subcontractors.length + 1 };
                syncToSheets('subcontractors', newItem);
                return { subcontractors: [...state.subcontractors, newItem] };
            }),
            updateSubcontractor: (id, data) => set((state) => ({
                subcontractors: state.subcontractors.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addSafetyRecord: (item) => set((state) => {
                const newItem = { ...item, id: state.safety.length + 1 };
                syncToSheets('safety', newItem);
                return { safety: [...state.safety, newItem] };
            }),
            updateSafetyRecord: (id, data) => set((state) => ({
                safety: state.safety.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addTimeEntry: (item) => set((state) => {
                const newItem = { ...item, id: state.timeEntries.length + 1 };
                syncToSheets('time_entries', newItem);
                return { timeEntries: [...state.timeEntries, newItem] };
            }),
            updateTimeEntry: (id, data) => set((state) => ({
                timeEntries: state.timeEntries.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
            ...
            addCost: (item) => set((state) => {
                const newItem = { ...item, id: state.costs.length + 1 };
                syncToSheets('costs', newItem);
                return { costs: [...state.costs, newItem] };
            }),
            updateCost: (id, data) => set((state) => ({
                costs: state.costs.map((i) => i.id === id ? { ...i, ...data } : i)
            })),
